...
while ConEntrada==True:
    a=input('insira os pontos(vetores de Rn) separados por vírgula:')
    a=a.replace(' ', '')
    if '/0,' not in a and '/0)' not in a:
        ConEntrada=False
        ...
        if b[0]!=b[1]:
            ...
            if dimini==1: #dimini=1 quer dizer que são vetores em r2, ou seja podemos escrever equação geral
                ...
                if vetornormal[0]<0:#Isto somente é visual para ter x com sinal positivo.
                    ...
                    print(' = {}'.format(c))              
            #imprimindo na tela o desenho abaixo
                if vetornormal[1]!=0:
                    varX=np.arange(-20,20,1)
                    #x=np.linspace(-100,100,1000)
                    plt.plot(varX,(c-vetornormal[0]*varX)/vetornormal[1])
                    plt.show()
                else:
                    varY=np.arange(-20,20,1)
                    varX=(c-vetornormal[1]*varY)/vetornormal[0]
                    plt.plot(varX,varY)
                    plt.show()
    ...
    if m==3:#neste caso tem-se três vetores
       if dimini==2:#dimini=2 indica que são três vetores em r3 usando o 'if' acima
        ...
        if n[0]==n[1]==n[2]==0:#neste caso o vetor normal é nulo ou seja os vetores v1 e v2 são colineares
            ...
            if n[2]!=0:
                varx,vary=np.mgrid[-20:20:500J, -20:20:500J]
                varz=(d-n[0]*varx-n[1]*vary)/n[2]
            elif n[1]!=0:
                varx,varz=np.mgrid[-20:20:500J, -20:20:500J]
                vary=(d-n[0]*varx-n[2]*varz)/n[1]
            else:
                vary,varz=np.mgrid[-20:20:500J, -20:20:500J]
                varx=(d-n[1]*vary-n[2]*varz)/n[0]

            fig = plt.figure(figsize=(7,7))
            ax=fig.add_subplot(projection='3d')
            ax.plot_surface(varx,vary,varz)
            plt.show()

    if m==dimini+1 and t==0:
      c=migsch(b)
      print('Os vetores obtidos no processo de Gram-Schmidt são:')
      for i in range(0, m):
          if minorm(c[i])!=0:
