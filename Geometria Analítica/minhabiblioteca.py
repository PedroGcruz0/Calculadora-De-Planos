...
def numtofrac(a):
    ctrden=0#Essa variável controla os casos a/0 pedindo que o usuário insira novamente o denominador 
    ctrsinal=1#Essa variável controla os casos de cadeias da forma -a/b para não considerar o sinal como um carater
    while(ctrden==0):#Este control pedira para você inserir a a/b desde que b=0
        fraction=list(a)
        ...
        if '/' not in fraction and '.' not in fraction: #Neste if, se a cadeia é simplesmente um número transformamos a cadeia em um inteiro.
            ...
            for i in range(0,len(fraction)):
                f1+=int(fraction[i])*10**(len(fraction)-1-i)
            ctrden=1#É necessário mudar o valor desta variável para não seguir repetindo o processo para sempre
            ...
            for i in range(0,c):#Transformando a cadeia numerador em um número.
                num+=int(fraction[i])*10**(c-1-i)
            den=0#armazea o denominador
            for i in range(c+1, len(fraction)):#Transforma a cadeia denominador em um número.
            ...
            if den==0:#Aviso no caso você tente ingressar uma fraçaõ com denominador nulo
                ...
                ctrsinal=1#É necessário voltar este control para 1 pois caso tu ingressa -3/0 ele teria armazenado -1 e na segunda iteração se ingressar a cadeia '3/2' ele iria devolver o número -3/2
            ...
            num=0
            for i in range(0,c):#transforma a cadeia a que pode ter várias casas por exemplo a='2344' no número a=2344
                num+=int(fraction[i])*10**(c-1-i)
            den=0#Trata a parte decimal ou seja a parte b da cadeia a.b para transforma-la em uma fração
            for i in range (c+1, len(fraction)):
                den+=int(fraction[i])*10**(len(fraction)-i-1)
            f1=num+Rational(den, 10**(len(fraction)-c-1))#Soma a parte inteira e a fração decimal
